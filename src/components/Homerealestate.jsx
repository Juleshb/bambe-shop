...
import Radio from "./assets/car.png"
  ...
  const [products, setProducts] = useState([]);
    ...
    useEffect(() => {
      const fetchProducts = async () => {
        try {
          const productsRes = await axiosInstance.get("/api/listings");
          setProducts(productsRes.data);
          const categories = await axiosInstance.get("/api/propertycategories");
...
<div className='  flex-col w-80 mt-20 pt-10 border-r-2 hidden lg:flex '>
  ...
  <Link className=' mb-4 hover:text-[#F15C26]' to="/">{category.category_name}</Link>
      </div>
    ))
  }
              ...
              <div className="bg-slate-400 rounded-md flex flex-col items-center p-4">
                ...
                <Link to={`/listing/${product.id}`}>
                  <img
                    ...
                    src={product.images?.length > 0 ? `https://bambe.shop${product.images[0].image_url}` : "placeholder-image-url"}
                    alt={product.name}
                  />
                </Link>
...
<p className=' text-3xl sm:text-4xl text-white font-bold mb-10'>Enhance Your Driving Experience</p>
